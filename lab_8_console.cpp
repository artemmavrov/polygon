...
long double calculate_heuristic(long double x1, long double y1, long double x2, long double y2) { //Откидывает очевидно не нужные пути, за счёт чего экономит время
...
long double AStar(int& start, int& end, vector<vector<long double>>& graf, unordered_map<string, int>& graf_map, int& n) {
    ...
    while (!open_set.empty()) { //O((V + E) * logV) в худшем(отработает как Дейкстера), O(E) в лучшем
...
int main() {
    ...
    string file_way = "C:/Users/artem/Desktop/labs/algoritms/lab-8/unweighted_graph.txt";
